...
import com.aliyun.homeshell.AnimationPagedView.Type;
import com.aliyun.homeshell.setting.ContinuousHomeShellReceiver;
/* YUNOS BEGIN */
// ##date:2014/06/30  ##author: xinzheng.lxz ##BugId:133822 add for native lifecard
/* YUNOS END */
...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    /* YUNOS BEGIN */
    // ##date:2014/3/18 ##author:xindong.zxd
    // Adjustment parameters that Slide the screen.
    ...
    public static boolean sContinuousHomeShellFeature = true;
    private static SharedPreferences mSharedPreferences;

    protected final Rect mInsets = new Rect();
    /** Runnables that will be executed when page stops moving. */
    private List<Runnable> mOnPageEndMovingRunnables = null;

    /** Used to temporarily prevent user scrolling page. */
    private boolean mScrollingEnable = true;

    //private SearchBridge mSearchBridge = null;
    private float mMeasuredWidth = 0;
    ...
    final private void init() {
        ...
        mMeasuredWidth = getResources().getDisplayMetrics().widthPixels;
    ...
    private boolean getContinuousHomeShellStatus() {
        return (mSharedPreferences != null)
                && mSharedPreferences.getBoolean(ContinuousHomeShellReceiver.KEY_CONTINUOUS_HOMESHELL_STYLE, true);
    }
    ...
    protected void pageEndMoving() {
        if (mIsPageMoving) {
            mIsPageMoving = false;
            onPageEndMoving();
            if (mOnPageEndMovingRunnables != null &&
                mOnPageEndMovingRunnables.isEmpty() == false) {
                for (Runnable runnable : mOnPageEndMovingRunnables) {
                    runnable.run();
                }
                mOnPageEndMovingRunnables.clear();
            }
        }
    }
    ...
    public void runOnPageStopMoving(Runnable runnable) {
        if (runnable == null) return;
        if (!mIsPageMoving) {
            // the page is not moving, execute immediately
            runnable.run();
        } else {
            // add to list and wait for pageEndMoving()
            if (mOnPageEndMovingRunnables == null) {
                mOnPageEndMovingRunnables = new ArrayList<Runnable>();
            }
            mOnPageEndMovingRunnables.add(runnable);
        }
    }
    ...
    public void setScrollingEnable(boolean enabled) {
    ...
    public void scrollTo(int x, int y) {
        ...
        mUnboundedScrollX = x;
        if(!(this instanceof Hideseat) && SearchBridge.isHomeShellSupportGlobalSearchUI(getContext())){
            mLauncher.getSearchBridge().onAnimationStart(SearchBridge.ANIMATION_TYPE_SLIDE_TO_LIFECARD);
            float motionX = 0;
            if(mCurrentPage == 0){
                if(x<0){//0->n-1
                   motionX = mMeasuredWidth+x;
                }
                else if(x >(getChildCount()-1)*mMeasuredWidth){
                    motionX  = x-(getChildCount()-1)*mMeasuredWidth;
                }
                else {
                    motionX = mMeasuredWidth -x;
                }
                float progress = motionX/mMeasuredWidth;
                progress = Math.abs(progress);
                mLauncher.getSearchBridge().onAnimationUpdate(progress);
            }
        ...
        //YUNOS BEGIN
        //author:chenjian.chenjian,BugID:7762700
        if(mCurrentPage == getChildCount()-1 && mMaxScrollX != x){//sometimes onMeasure will delay,need to update mMaxScrollX in advance
            final int index = isLayoutRtl() ? 0 : mCurrentPage;
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
            ...
            /* YUNOS BEGIN added by xiaodong #108519*/
        ...
        if (childCount > 0) {
            ...
            if (mPageSpacing == AUTOMATIC_PAGE_SPACING) {
                ...
                int spacing = Math.max(offset, widthSize - offset -
                        getChildAt(mLauncher.getWorkspace().getIconScreenHomeIndex())
                                .getMeasuredWidth());
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (!AgedModeUtil.isAgedMode()) {
            mCellLayoutMarginBottom = getContext().getResources().getDimensionPixelSize(
                    R.dimen.workspace_celllayout_margin_bottom);
        } else {
            mCellLayoutMarginBottom = getContext().getResources().getDimensionPixelSize(
        ...
        if (SearchBridge.isHomeShellSupportGlobalSearchUI(getContext())
            ...
            heighGlobalSearch = getContext().getResources().getDimensionPixelSize(
                    R.dimen.cell_layout_globalSearch_h);
        }
        /* YUNOS END */
            ...
            if (child.getVisibility() != View.GONE) {
                ...
                if (mCenterPagesVertically) {
                    ...
                    if (((CellLayout)child).isHideseatOpen) {
                        ...
                        /* YUNOS BEGIN PB */
                        // ##modules(SystemUI): ##author:yongxing.lyx
                        // ##BugID:(6638301) ##date:2015/11/18
                        // ##description: fix margin top to avoid flash when
                        // show or hide nav bar.
                        if (Launcher.hasNavigationBar()) {
                            int cellLayoutMarginTop = getContext().getResources()
                                    .getDimensionPixelSize(R.dimen.workspace_top_padding);
                            /* YUNOS BEGIN */
                            // ##date: 2016/08/11 ## author: yongxing.lyx
                            // ##BugID:8699757:search bar overlap with icons in
                            // phone with navbar.
                            childTop = cellLayoutMarginTop + mLauncher.getStatusBarHeight()
                                    + heighGlobalSearch;
                            /* YUNOS END */
                        } else {
                            childTop = height - childHeight - mCellLayoutMarginBottom
                                    + mLauncher.getCustomHideseat().getCustomeHideseatHeight();
                        }
                        /* YUNOS END PB */
                    } else {
                        //edit by dongjun for adjust child top according to child padding bottom
                        ...
                        if (mLauncher.isLeftScreen(i)) {
                            ...
                            if (Launcher.hasNavigationBar()) {
                                ...
                                /* YUNOS BEGIN */
                                // ##date: 2016/08/11 ## author: yongxing.lyx
                                // ##BugID:8699757:search bar overlap with icons
                                // in phone with navbar.
                                childTop = cellLayoutMarginTop + mLauncher.getStatusBarHeight()
                                        + heighGlobalSearch;
                                /* YUNOS END */
                            } else {
    ...
    /* YUNOS BEGIN */
    //author xiaodong.lxd 2013-11-30
    //re-fill mChildOffsets when childCount has changed
    ...
    protected int getChildOffset(int index) {
        ...
        /* YUNOS BEGIN */
        //author xiaodong.lxd 2013-11-29
        if(childOffsets != null && childOffsets.length <= index) {
            ...
            if (index >= childOffsets.length) {
                if (childOffsets.length == 0)
                    return 0;
                Log.e(TAG, "index out of bound index " + index + " childOffset.length " + childOffsets.length);
                return childOffsets[childOffsets.length - 1];
            }
    ...
    protected int getRelativeChildOffset(int index) {
        if (mChildRelativeOffsets != null && index >= mChildRelativeOffsets.length) {
            if (mChildRelativeOffsets.length == 0) {
                return 0;
            }
            return mChildRelativeOffsets[mChildRelativeOffsets.length - 1];
        }
        else if (mChildRelativeOffsets != null && mChildRelativeOffsets[index] != -1) {
            if (index >= mChildRelativeOffsets.length) {
                Log.e(TAG, "index out of bound index " + index + " mChildRelativeOffsets.length " + mChildRelativeOffsets.length);
                return mChildRelativeOffsets[mChildRelativeOffsets.length - 1];
            }
    ...
    protected int getScaledMeasuredWidth(View child) {
        ...
        //YUNOS BEGIN added by xiaodong.lxd #5201976 make sure measureWidth can be assigned a valid value
        int measuredWidth = -1;
        if(child == null) {
    ...
    protected int mPageAnimType;
    protected boolean is3DEffect(int effect) {
        if (effect == Type.GLASS_3D || effect == Type.CLOTH_3D) {
            return true;
        }
        return false;
    }
    ...
    protected void getVisiblePages(int[] range) {
        ...
        if (pageCount > 0) {
            ...
            while (leftScreen != endIndex && currPage.getX() + currPage.getWidth() < getScrollX()) {
                leftScreen += delta;
                currPage = getPageAt(leftScreen);
            }
            rightScreen = leftScreen;
            currPage = getPageAt(rightScreen + delta);
            while (rightScreen != endIndex && currPage.getX() < getScrollX() + screenWidth) {
                rightScreen += delta;
                currPage = getPageAt(rightScreen + delta);
            }
            if (sContinuousHomeShellFeature && getChildCount() > 1) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        // Skip touch handling if scrolling is disabled
        if (!mScrollingEnable) return super.onInterceptTouchEvent(ev);
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                if (finishedScrolling) {
                    ...
                    if ((xDist != 0 && xDist < mTouchSlop) || (newX != mUnboundedScrollX)) {
...
//        setScrollX(mOverScrollX);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        // Skip touch handling if scrolling is disabled
        if (!mScrollingEnable) return super.onTouchEvent(ev);
        ...
        case MotionEvent.ACTION_DOWN:
            ...
            if (!mScroller.isFinished()) {
                mScroller.abortAnimation();
                /*YUNOS BEGIN*/
                //xiaodong.lxd 2014-11-28 #5605457
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        /*YUNOS BEGIN modified by xiaodong.lxd #116358 then modified by wenliang.dwl in #130603*/
...
/*Commented by xiaodong.lxd #5605457
...
*/
    ...
    protected void snapToPage(int whichPage, int delta, int duration) {
        if (sContinuousHomeShellFeature && getChildCount() > 1) {
            ...
            boolean loopFlag = Math.abs(delta) >= loopDelta;
            /* YUNOS BEGIN xiaodong.lxd added 6626974 */
            // Press HOME-KEY to main screen, it's not a loop situation
            if (loopDelta == delta && getChildCount() == 2) {
                loopFlag = false;
            }
            /* YUNOS END */
            snapToPage(whichPage, delta, duration, loopFlag);
...
// ##date:2014/06/30  ##author: xinzheng.lxz ##BugId:133822 add for native lifecard
            if (immediately || index != (mPageIndicatorView.getCurrentPos())) {
                ...
                mPageIndicatorView.setCurrentPos(immediately ? mCurrentPage : index);
